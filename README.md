...
![界面布局图](readme-files/preview-layout.png)
...
![Slots位置图](readme-files/preview02.png)
...
|   名称   |          类型           |    默认值     |                   说明                    |
|:------:|:---------------------:|:----------:|:---------------------------------------:|
| survey | FreeSurveyCore.Survey | new Survey | 构建的问卷数据<br/>⚠️注意仅支持带参model，不支持默认v-model |
...
2. 预览区组件（UIElement）：这是在预览区中呈现题目或元素的组件，每个添加到问卷中的组件（问题或元素）都会使用这个组件呈现在预览区中，该组件会以 Prop 形式被自动注入当前问题的数据对象，Prop 的数据类型为`ElementProps`，为了完成数据的更新，还需要实现 Emit，具体的数据类型为`ElementEmits`。若要添加包含拖拽按钮和删除按钮等操作的操作栏，需要使用`element-with-operations-bar`组件包裹内容，它会自动添加操作栏并在鼠标悬停在题目上时显示，此外，还会在选中或鼠标悬停状态下显示一个边框。
